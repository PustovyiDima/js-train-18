   ...
   try {
      if (typeof data === "object") {
         if (Object.keys(data).length > 0) {
            return data;
         } else return "Об'єкт пустий";
      } else {
         const error = new TypeError("Аргумент не є об'єктом");
         throw error;
      }
   } catch (err) {
      console.log(err);
...
function getBookById(books, id) {
   ...
   try {
      let book = books.filter((obj) => obj.id === id);
      if (book.length <= 0) {
         const err = new TypeError(`Книга з ID ${id} не знайдена!`);
         throw err;
      }
      let res = book[0].title;
      return `Книга: ${res}`;
   } catch (error) {
      return error.message;
   }
...
function decodeURIComponentWrapper(encodedString) {
   ...
   try {
      let res = decodeURI(encodedString);
      return res;
   } catch (error) {
      const err = new URIError("Помилка декодування URI");
      return err;
   }
}
...
function evaluateExpression(expression) {
   ...
   try {
      ...
      const err = new EvalError("hello is not defined");
      console.log(err);
   }
}
